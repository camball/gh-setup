...
if [ $# -eq 0 ]; then
    ...
    while true; do
        ...
        case $RESPONSE in
            [Yy]* ) REPO_NAME="$DIR_TITLE_FROM_SUPPLIED_PATH"; break;;
            [Nn]* ) echo ""; read -p "Enter the title of the git/GitHub repository: " REPO_NAME; break;;
            * ) echo "Please answer Y or N.";;
        esac
    done
...
quit()" # weird indentation is to run python properly

            # test that the repository creation was successful
            if [ $? != 0 ]; then
                echo "Automatic repository creation failed..."
                manuallyCreateRepo
            fi
        fi
    fi
